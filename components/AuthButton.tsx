  ...
  return (
    <div className="flex flex-col items-center space-y-2">
      <button
        ...
        disabled={isProcessing || isLoading || !isProviderEnabled}
        title={!isProviderEnabled ? 'Провайдер пока не настроен' : undefined}
        className={`
      ...
      >
        ...
        {label ?? (provider === 'google' ? 'Войти через Google' : 'Войти через Яндекс')}
      </button>

      {!isProviderEnabled && provider === 'yandex' && (
        <div className="text-xs text-amber-600 dark:text-amber-400 text-center max-w-xs">
          Яндекс OAuth временно недоступен. Проверьте настройки провайдера в Supabase.
        </div>
      )}
