...
const RPM_LIMIT = 15; // Gemini Flash limit is approx 15 RPM for free tier

// Helper to clean up double escaped newlines sometimes returned by AI in JSON
const cleanTextResponse = (text: string): string => {
  if (!text) return '';
  // Replace literal string "\n" with actual newline character, avoiding double replacement
  return text.replace(/\\n/g, '\n');
};
...
export default function App() {
  ...
  useEffect(() => {
    initGA(process.env.GA_MEASUREMENT_ID);
  }, []);
  ...
  // Check quota every second to auto-decrement as time passes
  useEffect(() => {
    updateQuotaTracker(); // Initial check
  ...
  const [state, setState] = useState<AppState>(() => {
    try {
      ...
      if (saved) {
        ...
        // Deep merge/spread to ensure all fields exist even if schema changed slightly or storage is partial
        return {
  ...
  // Effect to save state to localStorage with debounce
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      try {
        ...
        // Fallback: Try saving without images if they are causing quota issues
        try {
          const cleanFormData = {
            ...state.formData,
          ...
          console.error('Failed to save cleaned state:', retryError);
        }
      }
    }, 500); // 500ms debounce
  ...
  // Scroll to result on success (or if loaded from storage)
  useEffect(() => {
  ...
  const handleCategoryChange = (id: CategoryId) => {
    ...
    setState(prev => ({
      ...
      keywords: [],
      error: null,
      validationErrors: {} // Clear errors on category switch
    }));
  };
  ...
  const updateFormData = (field: string, value: string) => {
    setState(prev => ({
      ...
      validationErrors: {
        ...
        [field]: false // Clear error when user types
      },
      formData: {
        ...prev.formData,
        [prev.category]: {
  ...
  const validateForm = (): boolean => {
    ...
    // Explicitly casting data to specific types to check fields safely
    switch (category) {
      case 'electronics': {
