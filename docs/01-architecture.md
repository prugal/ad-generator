...
**Как работает**:
- **База данных**: Созданы таблицы `user_credits` и `credit_transactions` в Supabase для отслеживания баланса и истории операций. По умолчанию, каждому новому пользователю начисляется 3 кредита.
- **Списание**: Кредиты списываются при генерации и оптимизации объявлений. Логика списания реализована в функциях `handleGenerate` и `handleOptimize` в `AdGenerator.tsx`, которые вызывают API-эндпоинт `/api/credits`.
- **API**: Созданы API-маршруты для управления кредитами: `/api/credits` (GET, POST).
- **Отображение**: Баланс кредитов отображается в интерфейсе рядом с кнопкой аутентификации.
